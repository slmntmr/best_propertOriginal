...
@Table(name = "countries") // Bu varlığın veritabanındaki tablo adını "countries" olarak belirtir.
@Data// Lombok'un @Data anotasyonu, bu sınıf için getter ve setter yöntemlerini, toString, equals ve hashCode gibi temel metodları otomatik olarak oluşturur.
@AllArgsConstructor
...
public class Country {
    ...
    private Long id; // Ülkenin benzersiz kimliği (PK).

    @Column(name = "name", nullable = false, length = 30) // Bu alanın boş olamayacağını ve maksimum uzunluğunu belirtir.
    private String name; // Ülkenin adı.

    @OneToMany(mappedBy = "country") // Country ile Advert arasında bire çok ilişki olduğunu belirtir.
    private List<Advert> adverts; // Bu ülkede bulunan ilanların listesi.

    @OneToMany(mappedBy = "country") // Country ile City arasında bire çok ilişki olduğunu belirtir.
    private List<City> cities; // Bu ülkedeki şehirlerin listesi.
}
